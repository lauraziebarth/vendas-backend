...
class BuscaTodosOsItensDeUmPedidoTests(TestCase):
    ...
    def test_busca_todos_os_itens_de_um_pedido(self):
        result = busca_todos_os_itens_de_um_pedido(self.pedido.id).count()
        result_list = busca_todos_os_itens_de_um_pedido(self.pedido.id)

        self.assertEqual(result, 4)
        self.assertEqual(result_list[0].id, 1)
        self.assertEqual(result_list[1].id, 2)
        self.assertEqual(result_list[2].id, 3)
        self.assertEqual(result_list[3].id, 4)
        self.assertEqual(result_list[1].preco_liquido, 30)
